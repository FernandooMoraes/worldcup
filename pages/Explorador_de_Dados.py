...
def load_enriched_dataset() -> pd.DataFrame:
    ...
    if candidates:
        ...
        path = DATA_DIR / "FIFA_ELO_DadosSelecoes_2026-04-15.xlsx"
...
st.markdown("## 📋 Data — Enriched Database")
st.caption(f"Source: `{dataset_name}` · {len(df_raw)} selections · {len(df_raw.columns)} variables")
# ============ MÉTRICAS RESUMO ============
col_m1, col_m2, col_m3, col_m4, col_m5 = st.columns(5)

with col_m1:
    st.metric("🌍 Teams", len(df_raw))
with col_m2:
    st.metric("🏆 Groups", df_raw["Grupo"].nunique() if "Grupo" in df_raw.columns else "—")
with col_m3:
    if "Valor_Mercado_Milhoes_EUR" in df_raw.columns:
        ...
        st.metric("💰 Total Value", f"€{total_market:,.0f}M")
    else:
...
with col_filter_1:
    ...
    if conf_col:
        all_confs = ["All"] + sorted(df_raw[conf_col].dropna().unique().tolist())
        filtro_conf = st.selectbox("Confederation", all_confs, key="dados_filtro_conf")
    else:
        filtro_conf = "All"

with col_filter_2:
    if "Grupo" in df_raw.columns:
        all_groups = ["All"] + sorted(df_raw["Grupo"].dropna().unique().tolist())
...
with col_filter_3:
    if "Continente_Geo" in df_raw.columns:
        all_continents = ["All"] + sorted(df_raw["Continente_Geo"].dropna().unique().tolist())
        filtro_cont = st.selectbox("Continent", all_continents, key="dados_filtro_cont")
    else:
        filtro_cont = "All"

# Aplicar filtros
df = df_raw.copy()
if filtro_conf != "All" and conf_col:
    df = df[df[conf_col] == filtro_conf]
if filtro_grupo != "All" and "Grupo" in df.columns:
    df = df[df["Grupo"] == filtro_grupo]
if filtro_cont != "All" and "Continente_Geo" in df.columns:
...
def build_model_variables(dataframe: pd.DataFrame) -> pd.DataFrame:
    ...
    team_column = first_existing_column(dataframe, ["Teams", "Teams"])
    if team_column:
        hosts = {"United States", "Mexico", "MÃ©xico", "Canada", "CanadÃ¡"}
...
COLUMN_GROUPS = {
    "Identificação": ["Teams", "Grupo", "Confederação", "Confederacao", "Continente_Geo", "Capital", "Regiao"],
...
DEFAULT_DISPLAY = [
    "Teams", "Grupo", "FIFA_Current_Rank", "FIFA_Current_Points",
    "ELO_Ranking", "ELO_Rating", "ELO_Chg_1A",
    "Valor_Mercado_Milhoes_EUR", "Participações_Copa_Mundo",
]
...
with st.expander("⚙️ Select visible columns", expanded=False):
...
display_df = display_df.rename(
    columns={
        "Teams": "Teams",
        "Grupo": "Group",
        "Valor_Mercado_Milhoes_EUR": "Market Value (€M)",
        "Participações_Copa_Mundo": "World Cup Participations",
        "FIFA_Current_Rank": "FIFA Rank",
        "FIFA_Current_Points": "FIFA Points",
        "ELO_Ranking": "ELO Rank",
        "ELO_Rating": "ELO Rating",
        "ELO_Chg_1A": "ELO Change (1Y)"
    }
)
...
with viz_tab1:
    ...
    with col_v1:
        if "ELO_Rating" in df.columns and "Teams" in df.columns:
            df_elo_sorted = df.nlargest(48, "ELO_Rating")
            fig_elo = px.bar(
                df_elo_sorted,
                y="Teams", x="ELO_Rating",
                orientation="h",
                color="ELO_Rating",
    ...
    with col_v2:
        if "FIFA_Current_Points" in df.columns and "Teams" in df.columns:
            df_fifa_sorted = df.nlargest(48, "FIFA_Current_Points")
            fig_fifa = px.bar(
                df_fifa_sorted,
                y="Teams", x="FIFA_Current_Points",
                orientation="h",
    ...
    with col_v3:
        if "Valor_Mercado_Milhoes_EUR" in df.columns and "Teams" in df.columns:
            df_mkt = df.nlargest(48, "Valor_Mercado_Milhoes_EUR")
            fig_mkt = px.bar(
                df_mkt,
                y="Teams", x="Valor_Mercado_Milhoes_EUR",
                orientation="h",
                color="Valor_Mercado_Milhoes_EUR",
                color_continuous_scale=["#1d1b0f", "#7AB802", "#FFCF26"],
                title="Top 48 — Market value (€M)",
            )
    ...
    with col_v4:
        if "ELO_Chg_1A" in df.columns and "Teams" in df.columns:
            df_mom = df.copy()
            ...
            fig_mom.update_layout(
                title="Top 48 — ELO Variation (1 Year)",
                plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)",
                font_color="#C9D1C9", title_font_color="#68E70F",
...
with viz_tab2:
    ...
    if model_variables_df.empty:
        st.info("None of the model variables were found in the filtered dataset.")
    else:
...
with viz_tab3:
    ...
    if corr_df.shape[1] < 2:
        st.info("At least two valid variables are needed to calculate correlations.")
    else:
        team_column = first_existing_column(df, ["Teams", "SeleÃ§Ã£o"])
