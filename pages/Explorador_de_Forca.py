...
def find_latest_enriched_dataset() -> Path:
    candidates = sorted(DATA_DIR.glob("FIFA_ELO_DadosSelecoes_*.xlsx"))
    if candidates:
        return candidates[-1]
    fallback = DATA_DIR / "FIFA_ELO_DadosSelecoes_2026-04-15.xlsx"
    return fallback
...
def load_force_table(dataset_path: str) -> pd.DataFrame:
    ...
    result = df.copy()
    result["team_key"] = result["Teams"].map(canonical_team_key)
    
    hosts = ["Estados Unidos", "México", "Canadá"]
    result["is_host"] = result["Teams"].isin(hosts).astype(int)
...
def generate_group_predictions(
    ...
    for match in schedule:
        ...
        if team_a_str in dataframe["Teams"].values and team_b_str in dataframe["Teams"].values:
            forca_a = float(dataframe.loc[dataframe["Teams"] == team_a_str, "forca_com_offset"].iloc[0])
            forca_b = float(dataframe.loc[dataframe["Teams"] == team_b_str, "forca_com_offset"].iloc[0])
...
def run_complete_simulation_progressive(
    ...
    def update_progress(completed: int, total: int) -> None:
        status_placeholder.markdown(f"**Detailed progress:** {completed:,} / {total:,} cups")

    with st.spinner("Processing complete simulation..."):
    ...
    status_placeholder.success(f"Full simulation completed: {n_sims:,} cups!")
...
st.markdown("## 2026 World Cup Simulation")
st.markdown(
    """
<p style="font-size: 1rem; margin-bottom: 1.5rem;">
Combine the normalized indices of <b>FIFA</b>,
<b>ELO</b>, o <b>recent moment</b>, <b>market value</b>,
o <b>history in Cups</b>, ajuste <b>elasticity</b> e <b>offset</b> And see how this changes the probability of a game.
</p>
""",
...
try:
    ...
    st.error(f"Error loading enriched database: {error}")
    st.stop()

with st.sidebar:
    st.markdown("#### Composition of the Strength Indicator")
    col1, col2 = st.columns(2)
    with col1:
        weight_fifa = st.slider("FIFA", min_value=0.0, max_value=1.0, value=DEFAULT_WEIGHT_FIFA, step=0.01)
    with col2:
        weight_market = st.slider("Market", min_value=0.0, max_value=1.0, value=DEFAULT_WEIGHT_MARKET, step=0.01)
        
    col3, col4 = st.columns(2)
    with col3:
        weight_elo = st.slider("ELO", min_value=0.0, max_value=1.0, value=DEFAULT_WEIGHT_ELO, step=0.01)
    with col4:
        weight_momentum = st.slider("Moment", min_value=0.0, max_value=1.0, value=DEFAULT_WEIGHT_MOMENTUM, step=0.01)

    col5, col6 = st.columns(2)
    with col5:
        weight_history = st.slider("Cups History", min_value=0.0, max_value=1.0, value=DEFAULT_WEIGHT_HISTORY, step=0.01)
    with col6:
        weight_host = st.slider("Host", min_value=0.0, max_value=1.0, value=DEFAULT_WEIGHT_HOST, step=0.01)

    st.markdown("---")
    st.markdown("#### Model Parameters")
    
    media_gols = st.slider("Average goals per match", min_value=0.5, max_value=5.0, value=DEFAULT_MEDIA_GOLS, step=0.05)

    col7, col8 = st.columns(2)
    with col7:
        offset = st.slider("Offset", min_value=0.0, max_value=1.0, value=DEFAULT_OFFSET, step=0.01)
    with col8:
        elasticidade = st.slider("Elasticity", min_value=0.1, max_value=5.0, value=DEFAULT_ELASTICIDADE, step=0.01)
    ...
    with col10:
        rho_dixon_coles = st.slider("Rho parameter", min_value=-0.30, max_value=0.00, value=DEFAULT_RHO_DIXON_COLES, step=0.01, disabled=not usar_dixon_coles)

    st.markdown("---")
    st.markdown("#### Simulation Parameters")
    n_sims = st.slider("Number of Cups", min_value=10000, max_value=1000000, value=10000, step=10000)
    tipo_chaveamento = st.radio("", ["Official Draw", "Random Draw"], horizontal=True)
    tipo_simulacao = st.radio("Simulation type",["Basic", "Complete"], horizontal=True)
...
if weight_sum <= 0:
    st.warning("The sum of the weights is zero. Adjust at least one weight to build up the resultant force.")
...
with col_m1:
    st.metric("Effective FIFA", f"{effective_fifa:.1%}")
with col_m2:
    st.metric("Effective ELO", f"{effective_elo:.1%}")
with col_m3:
    st.metric("Effective Momentum", f"{effective_momentum:.1%}")
with col_m4:
    st.metric("Effective Market", f"{effective_market:.1%}")
with col_m5:
    st.metric("Effective History", f"{effective_history:.1%}")
with col_m6:
    st.metric("Effective Host", f"{effective_host:.1%}")

st.markdown("### Strength Table")
...
columns={
    ...
    "fifa_force_01": "FIFA",
    "elo_force_01": "ELO",
    "momentum_force_01": "Momentum",
    "market_force_01": "Market",
    "world_cup_history_01": "History",
    "is_host": "Host",
    "forca_resultante_01": "Strength",
    "forca_com_offset": "Adjusted Strength",
...
team_options = combined_df["Teams"].tolist()
...
with col_left:
    ...
    home_flag = combined_df.loc[combined_df["Teams"] == home_team, "Link_Bandeira"].iloc[0]
    away_flag = combined_df.loc[combined_df["Teams"] == away_team, "Link_Bandeira"].iloc[0]
...
if home_team == away_team:
    with col_left:
        st.info("Choose two different selections to calculate the odds of the match.")
else:
    home_row = combined_df.loc[combined_df["Teams"] == home_team].iloc[0]
    away_row = combined_df.loc[combined_df["Teams"] == away_team].iloc[0]
...
<div class="match-stat-card" style="border-left: 3px solid #209927;">
    <div class="match-card-label" style="font-size: 0.82rem; font-weight: 600;">Strength</div>
...
""",
        ...
        with col_hm2:
            st.markdown(
                f"""
<div class="match-stat-card" style="border-left: 3px solid #209927;">
    <div class="match-card-label" style="font-size: 0.82rem; font-weight: 600;">Expected goals</div>
...
<div class="match-prob-card match-prob-card--draw" style="border: 2px solid #9e9e9e; box-shadow: 0 2px 12px rgba(158,158,158,0.12);">
    <div class="match-card-label match-draw-label">Draw</div>
...
""",
    ...
    with col_right:
        ...
        fig_heatmap.update_layout(
            title=dict(text="Probability of Scores", x=0.5, xanchor="center", font=dict(size=20)),
            xaxis=dict(
